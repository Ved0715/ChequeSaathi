...
import { chequeAPI, Cheque, ChequeType, ChequeDirection } from '@/lib/api/cheques';
...
export function EditChequeDialog({ cheque, open, onOpenChange, onSuccess }: EditChequeDialogProps) {
  ...
  useEffect(() => {
    if (cheque) {
      reset({
        ...
        issueDate: new Date(cheque.issueDate).toISOString().split('T')[0],
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      setIsLoading(true);
      await chequeAPI.update(cheque.id, {
        ...data,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update cheque details. All fields marked with * are required.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isLoading}>
              {isLoading ? 'Updating...' : 'Update Cheque'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
